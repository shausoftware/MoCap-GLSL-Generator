...
public class C3dDataParser {
    ...
    public static DataIndex parseGroupParameterData(byte[] parametersBlock, int idx, ByteOrder byteOrder) throws ParserException {
        ...
        if (groupId == 0) {
            //End parsing token detected
            return new DataIndex(null, idx, false);
        }
        ...
        if (groupId < 0) {
            ...
            if (increment == 0) {
                //No group data token detected - stop parsing
                return new DataIndex(null, idx, false);
            }
            next += increment;
            ...
            C3dGroup group = C3dGroup.builder().id(groupId).name(name).description(description).build();
            return new DataIndex<>(group, next, true);
            ...
            int increment = BinaryHelper.readInt16(word, byteOrder);
            if (increment == 0) {
                //No parameter data token detected - stop parsing
                return new DataIndex(null, idx, false);
            ...
            C3dParameter parameter = new C3dParameter(name,
                            ...
                            dimensionSizes,
                            values);
            return new DataIndex(parameter, next, true);
        }
    }
